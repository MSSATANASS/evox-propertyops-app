...
function ExpenseDialog({
  ...
  const handleSave = async () => {
    if (!form.propertyId || !form.description || !form.amount || !form.requestedBy) {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 pt-2">
          ...
          <Field label="Description">
            <Input value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="Plumbing parts" />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Amount (MXN)">
              <Input type="number" value={form.amount} onChange={(e) => set("amount", e.target.value)} placeholder="1850" />
            </Field>
            <Field label="Date">
          ...
          <Field label="Requested by">
            <Input value={form.requestedBy} onChange={(e) => set("requestedBy", e.target.value)} placeholder="John Smith" />
          </Field>
...
export function AddExpenseButton() {
  ...
  return (
    <ExpenseDialog
      ...
      onSave={async (f) => {
        await create({
          propertyId: f.propertyId as Id<"properties">,
...
export function EditExpenseButton({ expense }: { expense: Doc<"expenses"> }) {
  ...
  return (
    <ExpenseDialog
      ...
      initial={{ ...expense, amount: String(expense.amount), propertyId: expense.propertyId }}
      onSave={async (f) => {
        await update({
          id: expense._id,
          propertyId: f.propertyId as Id<"properties">,
          ...
          requestedBy: f.requestedBy,
          date: f.date,
          ...(expense.approvedBy ? { approvedBy: expense.approvedBy } : {}),
        });
...
export function DeleteExpenseButton({ id }: { id: Id<"expenses"> }) {
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button
            ...
            onClick={async () => { await remove({ id }); setOpen(false); toast.success("Expense deleted"); }}
          >
            Delete
          </Button>
        </div>
