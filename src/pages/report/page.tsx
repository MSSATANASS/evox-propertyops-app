...
import { Printer, Building2, CheckCircle2, Clock, Wrench, Receipt } from "lucide-react";
...
export default function ReportPage() {
  ...
  const loading = properties === undefined || allTasks === undefined || allExpenses === undefined;

  // Default to first property when loaded
  const effectiveId = selectedPropertyId ?? properties?.[0]?._id ?? null;
  const property = properties?.find((p) => p._id === effectiveId) ?? null;

  const tasks = allTasks?.filter((t) => t.propertyId === effectiveId) ?? [];
  const expenses = allExpenses?.filter((e) => e.propertyId === effectiveId) ?? [];
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          {loading ? (
            ...
            <Select
              ...
              onValueChange={(val) => setSelectedPropertyId(val as Id<"properties">)}
            >
      ...
      {loading ? (
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white/60 text-xs">by Evox · Professional Property Management</p>
                </div>
              </div>
              <div className="text-right">
                <p className="font-semibold text-sm">Monthly Management Report</p>
                <p className="text-white/70 text-xs mt-0.5">Period: August 2026</p>
              </div>
            </div>
          </div>
          ...
          <div className="px-8 py-6 space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="rounded-lg border border-border p-4">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Owner</p>
                <p className="text-xl font-bold text-foreground">{property.owner}</p>
              ...
              <div className="rounded-lg border border-border p-4 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-[oklch(0.25_0.08_250)]">{formatMXN(property.monthlyRent)}</span>
                </div>
                <p className="text-xs text-muted-foreground pt-1 border-t border-border">{property.address}</p>
              </div>
            </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                {[
                  { label: "Total", value: tasks.length, icon: Wrench, color: "text-foreground" },
                  { label: "Completed", value: completedTasks.length, icon: CheckCircle2, color: "text-green-600" },
                  { label: "Pending", value: pendingTasks.length, icon: Clock, color: "text-orange-500" },
                ].map((stat) => (
              ...
              {completedTasks.length > 0 && (
                <div>
                  <p className="text-sm font-semibold mb-2 text-foreground">Tasks completed this period:</p>
                  <div className="rounded-lg border border-border overflow-hidden">
                    <table className="w-full text-sm">
                      <thead className="bg-muted/50">
                        <tr>
                          <th className="text-left px-4 py-2 text-xs font-semibold text-muted-foreground">Task</th>
                          <th className="text-left px-4 py-2 text-xs font-semibold text-muted-foreground">Technician</th>
                          <th className="text-left px-4 py-2 text-xs font-semibold text-muted-foreground">Date</th>
                        </tr>
                      </thead>
                      <tbody>
                        {completedTasks.map((task, i) => (
                          <tr key={task._id} className={i % 2 === 0 ? "bg-white" : "bg-muted/20"}>
                            <td className="px-4 py-2.5 font-medium">{task.title}</td>
                            <td className="px-4 py-2.5 text-muted-foreground">{task.assignedTo}</td>
              ...
              {(pendingTasks.length > 0 || inProcessTasks.length > 0) && (
                <div className="mt-3">
                  <p className="text-sm font-semibold mb-2 text-foreground">Tasks in progress or pending:</p>
                  <div className="space-y-1.5">
                    {[...inProcessTasks, ...pendingTasks].map((task) => (
                      <div key={task._id} className="flex items-center justify-between px-4 py-2 rounded-lg bg-muted/30 text-sm">
                        <span>{task.title}</span>
                        <span className={`text-[10px] px-2 py-0.5 rounded font-medium ${statusColor[task.status] ?? ""}`}>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="rounded-lg border border-border p-3">
                  ...
                  <p className="text-xl font-bold text-green-600">{formatMXN(totalApproved)}</p>
                </div>
                <div className="rounded-lg border border-border p-3">
                  <p className="text-xs text-muted-foreground">Pending approval</p>
                  <p className="text-xl font-bold text-orange-500">{formatMXN(totalPending)}</p>
                </div>
              </div>
              ...
              {expenses.length > 0 && (
                <div className="rounded-lg border border-border overflow-hidden">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {expenses.map((exp, i) => (
                        <tr key={exp._id} className={i % 2 === 0 ? "bg-white" : "bg-muted/20"}>
                          ...
                          <td className="px-4 py-2.5 text-right font-semibold">{formatMXN(exp.amount)}</td>
                          <td className="px-4 py-2.5">
                            <span className={`text-[10px] px-2 py-0.5 rounded font-medium ${statusColor[exp.status] ?? ""}`}>
                    ...
                    <tfoot className="border-t border-border bg-muted/30">
                      <tr>
                        <td colSpan={2} className="px-4 py-2.5 text-sm font-semibold">Period total</td>
                        <td className="px-4 py-2.5 text-right font-bold">
                          {formatMXN(expenses.reduce((s, e) => s + e.amount, 0))}
                        </td>
                        <td />
                      </tr>
                    </tfoot>
        ...
        <p className="text-center text-muted-foreground py-12">No properties available.</p>
      )}
    </div>
  );
}
