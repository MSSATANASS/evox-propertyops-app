...
function TaskDialog({
  ...
  const handleSave = async () => {
    if (!form.propertyId || !form.title || !form.assignedTo) {
    ...
    <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (o) setForm(initial ?? empty); }}>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 pt-2">
          ...
          <Field label="Title">
            <Input value={form.title} onChange={(e) => set("title", e.target.value)} placeholder="Bathroom pipe leak" />
          </Field>
          <Field label="Description">
            <Textarea value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="Problem details..." rows={2} className="resize-none" />
          </Field>
          ...
          <Field label="Assigned to">
            <Input value={form.assignedTo} onChange={(e) => set("assignedTo", e.target.value)} placeholder="John Smith" />
          </Field>
...
export function EditTaskButton({ task }: { task: Doc<"tasks"> }) {
  ...
  return (
    <TaskDialog
      ...
      initial={{ ...task, photoUrl: task.photoUrl ?? "", propertyId: task.propertyId }}
      onSave={async (f) => {
        await update({
          id: task._id,
          propertyId: f.propertyId as Id<"properties">,
...
export function DeleteTaskButton({ id }: { id: Id<"tasks"> }) {
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button
            ...
            onClick={async () => { await remove({ id }); setOpen(false); toast.success("Task deleted"); }}
          >
            Delete
          </Button>
        </div>
